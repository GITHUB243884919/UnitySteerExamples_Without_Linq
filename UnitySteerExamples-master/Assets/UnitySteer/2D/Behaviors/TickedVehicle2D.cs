...
namespace UnitySteer2D.Behaviors
{
    ...
    /// 
    /// 1、聚合了TickedObject，持有了UnityTickedQueue，并把自己加入UnityTickedQueue
    /// 2、把各个steering上的力按权累加，算出合力。然后除以质量（默认1），计算出新的加速度。
    ///    各steering上的力由其派生类实现，不同的实现决定了不同的操控（AI）.
    ///    不知道为什么这里用的速度的单词来表示加速度。不过最终计算加速度的函数
    ///    ApplySteeringForce()是正确的。
    /// 3、在其Update函数中里处理了质点位移变化和方向变化，也就是移动了质点，不知道这是否合适。
    /// 
    /// 
    /// </summary>
    public abstract class TickedVehicle2D : Vehicle2D
    {
        ...
        // 去掉多余的变量
        // 下面有一行定义了一个TickedObject类型变量TickedObject
        // 并且也有了get set，实际代码中的用的也是 TickedObject
        // 因此这里的_tickedObject是多余的
...
#region Velocity / Speed methods
        ...
        /// <summary>
        /// 计算当前质点所持有的多个Steering组件，按权重计算其合力
        /// （如果当前质点上同时存在多个操控AI的话，就可能是多个Steering组件）
        /// </summary>
