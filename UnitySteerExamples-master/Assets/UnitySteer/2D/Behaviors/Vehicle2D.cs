...
namespace UnitySteer2D.Behaviors
{
    ...
    /// 对方向做了一些规定，加载了所有的steering类，还提供了一些计算工具函数。
    /// </summary>
    ...
    public abstract class Vehicle2D : DetectableObject2D
    {
        ...
        protected override void Awake()
        {
            ...
            int steeringSize = 0;
            int postSize     = 0;
            List<int> steeringIdxList = new List<int>();
            List<int> postIdxList     = new List<int>();
            for (int i = 0; i < allSteerings.Length; i++)
            {
                if (!allSteerings[i].IsPostProcess)
                {
                    steeringIdxList.Add(i);
                    steeringSize++;
                }
                else
                {
            ...
            Steerings = new Steering2D[steeringSize];
            SteeringPostprocessors = new Steering2D[postSize];
            for (int i = 0; i < steeringIdxList.Count; i++)
            {
                Steerings[i] = allSteerings[steeringIdxList[i]];
            }
            for (int i = 0; i < postIdxList.Count; i++)
            {
                SteeringPostprocessors[i] = allSteerings[postIdxList[i]];
            }
        ...
        /// 
        /// 判断另一质点是否在当前质点的观察半径内，并且这个算法
        /// 除了可以除了实现典型的雷达型可视区域之外，还实现了带角度的可视区域
        /// </summary>
        /// <param name="other">
        ...
        /// 
        /// 计算目标速度向量，取速度取值等于目标速度减去当前速度，控制在正负MaxForce之间。
        /// </summary>
        /// <returns>
        ...
        /// between them
        /// 
        /// 预测一段时间内当前质点和另一质点之间的距离。
        /// 两者可能是运动的，也可能是静止的，到底是运动还是静止取决于各自的速度。
        /// 
        /// </summary>
        /// <returns>
        /// Distance between positions
